...
/**
 * Powiadomienia IN-APP (wiersze w tabeli `Notification` + dzwonek na froncie).
 * Warstwa zapisu (`create`/`notifyStudents`) jest wołana z orkiestratora
 * `NotificationsService` (który równolegle wysyła maile) oraz bezpośrednio dla
 * zdarzeń wyłącznie in-app (nowa płatność, odwołane zajęcia). Warstwa odczytu
 * (`findAll`/`unreadCount`/`markRead`) obsługuje `NotificationsController`.
 */
@Injectable()
