...
export function RecurringClassModal({ open, onClose }: Props) {
  ...
  const [groupId, teacherId, dateFrom, dateTo, timeStart] = watch([
    "groupId",
    "teacherId",
    "dateFrom",
    "dateTo",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
          ...
          {previewDates.length > 0 && (
            <div className="bg-violet-50 rounded-xl px-4 py-3 space-y-2">
              <p className="text-sm font-medium text-violet-700">
                Zostanie utworzone {previewDates.length}{" "}
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {isSubmitting
                ? "Tworzenie..."
                : `Utwórz ${previewDates.length > 0 ? `(${previewDates.length})` : ""}`}
            </Button>
          </div>
        </form>
      </DialogContent>
