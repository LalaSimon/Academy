  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {group.level && (
          <span className="text-[11px] px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 border border-amber-500/20 font-medium">
            {group.level}
          </span>
        )}
      </div>
      ...
      <div className="pt-2 border-t border-border/50">
        ...
        <div className="h-1.5 rounded-full bg-muted/40 overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min(100, (group._count.students / group.maxStudents) * 100)}%`,
...
export default function StudentGroupsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-foreground">Moje grupy</h1>
        <p className="text-sm text-muted-foreground mt-0.5">
          {groupIds.length} {groupIds.length === 1 ? 'grupa' : 'grupy'}
        </p>
      </div>
      ...
      {!isLoading && groupIds.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-sm text-muted-foreground">Nie jesteś jeszcze przypisany do żadnej grupy</p>
        </div>
      )}
